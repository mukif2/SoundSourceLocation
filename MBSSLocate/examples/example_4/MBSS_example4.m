% File MBSS_example4.m
%
% Script example to  :
%    - Compute a simulated mixture of one moving source by a moving
...
isArrayMoving   = true; % The microphone array is static
subArray        = [];    % Do not use a sub array of microphones
...
%% Generate (using roomsimove) or load the 16 kHz simulated mixture
...
if(~exist('x.wav','file'))
    ...
    for i = 1:nsrcMixture
        ...
        [time,HH] = MBSS_roomsimove(fs,roomStruct.room_size,roomStruct.F_abs',roomStruct.A',sensorsStruct.sensor_xyz,sensorsStruct.sensor_off',sensorsStruct.sensor_type,sourcesStruct(i).sceneTimeStamps,sourcesStruct(i).source_xyz);
    ...
    warning('In order to reduce the simulation time, the signal is loaded from the previous generated wav file. Please manually erase x.wav if you want to change simulation parameters');
    x = audioread('x.wav');
end

%% Convert algorithm paramaters to MBSS structure
...
%% Results evaluation
fprintf('\nResults evaluation:\n');
nBlocks = size(azEst,1);

% Define the srcPos matrix of real source positions over the time
srcPos = zeros(3,nsrcMixture,size(sourcesStruct(1).source_xyz,2));
for i = 1:nsrcMixture
    srcPos(:,i,:) = sourcesStruct(i).source_xyz;
end

MBSS_eval(evalMode,angleThreshold,sMBSSParam.arrayCentroid,srcPos,azEst,elEst,block_timestamps,sceneTimeStamps);